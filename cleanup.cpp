...
void find_max(const std::array<std::array<float, 257>, 192>& data, int NBINS, float& maximum) {
	maximum = 0;
...
void same_mode_convolve(std::array<float, 204>& input1) {
	int size1 = 204;
	int size2 = 3;
	int size_out = size1;
	static constexpr std::array<float, 3> input2 = { 1.,1.,1. };

	// Zero-pad the input std::array
	static std::array<float, 3 + 204 - 1> input1_padded;
	std::copy(input1.begin(), input1.end(), input1_padded.begin() + (size2 - 1) / 2);

	// Reverse the second input std::array
	std::array<float, 3> reversed_input2;
...
inline void linear_size_interpolation(std::array<float, 37>& d) {
	float dx = d[36] - d[0];
	for (int i = 0; i < 37; i++) {
		d[i] = (d[i] - d[0]) / dx;
	}
...
//compiles
float correlationCoefficient(const std::array<float, 257>& X, const std::array<float, 257>& Y, int size) {
	float sum_X = accumulate(X.begin(), X.begin() + size, 0.0);
	float sum_Y = accumulate(Y.begin(), Y.begin() + size, 0.0);
	float sum_XY = inner_product(X.begin(), X.begin() + size, Y.begin(), 0.0);
	float squareSum_X = inner_product(X.begin(), X.begin() + size, X.begin(), 0.0);
	float squareSum_Y = inner_product(Y.begin(), Y.begin() + size, Y.begin(), 0.0);

	float corr = (size * sum_XY - sum_X * sum_Y) /
...
void fast_entropy(std::array<std::array<float, 257>, 192>& data, std::array<float, 192>& entropy_unmasked, std::array<float, 257>& logit, int NBINS) {
	static std::array<float, 257> d_subset;

	for (int i = 0; i < 192; i++) {
		std::array<float, 257>& d = data[i];
		...
		float dx = d_subset[NBINS - 1] - d_subset[0];
		for (int j = 0; j < NBINS; j++) {
			d_subset[j] = (d_subset[j] - d_subset[0]) / dx;
		}

		float v = correlationCoefficient(d_subset, logit, NBINS);
		if (isnan(v)) {
...
float determine_entropy_maximum(const std::array<float, 257>& logit, int NBINS) {
	std::array<float, 257> d = {};
	d[NBINS - 1] = 1.0;
	std::array<float, 257> logit_subset;
...
void fast_peaks(std::array<std::array<float, 257>, 192>& stft_, std::array<int, 192>& entropy, float thresh, std::array<float, 204>& entropy_padded, float& entropy_maximum, int& NBINS, std::array<std::array<float, 257>, 192>& mask) {
	float constant, test, thresh1 = 0.0;
...
//compiles
static void stft(std::array<float, 25087>& xp, const std::array<float, 512>& window, std::array<std::array<std::complex<float>, 257>, 192>& out) {

	static std::array<float, 257> Sx_temp_2;
	static std::array<float, 512> Sx_temp;
...
static void istft(std::array<std::array<std::complex<float>, 257>, 64>& Sx, std::array<float, 384>& buffer, std::array<float, 8192>& output, const std::array<float, 512>& window) {
	static std::array<float, 257> temp;
...
static void generate_true_logistic(std::array<float, 257>& logit, int points) {
	logit.fill(0.0);
	if (points == 1) {
		logit[0] = 1.0;
		return;
	...
	for (int i = 0; i < points; i++) {
		logit[i] = static_cast<float>(i) / (points - 1);
	}
	...
	float min_val = logit[0];
	float max_val = logit[points - 1];
	for (int i = 0; i < points; i++) {
...
public:
		...
		float t, initial, multiplier, MAXIMUM = 0 ;
	...
	std::array<float, 8192> process(std::array<float, 8192> input, float constant = 0.057, int NBINS = 37) {
		...
		std::size_t count = input.size() * sizeof(float);
