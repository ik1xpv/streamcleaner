...
void same_mode_convolve(std::array<double, 204>& input1) {
	int size1 = 204;
	int size2 = 3;
	int size_out = size1;
	static constexpr std::array<double, 3> input2 = { 1.,1.,1. };
...
public:
		...
		std::array<std::array<double, 257>, 192> stft_real = {};
		std::array<std::array<double, 257>, 192> smoothed = {};
		std::array<std::array<double, 257>, 192> previous = {};
	...
	std::array<double, 8192> process(std::array<double, 8192> input, double constant = 0.057, int NBINS = 37) {
		...
		if (set == false) {
			if (NBINS == 37) {//let's fill the std::array
				...
				generate_true_logistic(logit_distribution, NBINS);
				MAXIMUM = determine_entropy_maximum(logit_distribution, NBINS);
				NBINS_last = NBINS;
				set = true;
		...
		fast_entropy(stft_real, entropy_unmasked, logit_distribution, NBINS);
		...
		copy(begin(entropy_unmasked), end(entropy_unmasked), end(entropy_padded) - 198); //copy into middle of padding

		same_mode_convolve(entropy_padded);
		for (int i = 0; i < 192; i++) {
			entropy_padded[i + 6] = entropy_padded[i + 6] / 3;
		}

		for (int i = 6; i < 198; i++) {
			if (entropy_padded[i] < constant) {
				entropy_thresholded[i - 6] = 0;
			}
			else {
				entropy_thresholded[i - 6] = 1;
				count++;
				if ((i > 38) && (i < 166)) {
					truecount++;
				}
		...
		if (count > 0) {
			...
			if (truecount > 22 || longestConsecutive(entropy_thresholded, 32, 160) > 16) {
				...
				find_max(stft_real, NBINS, initial);
				sawtooth_filter(stft_real, scratch, smoothed, NBINS);
				fast_peaks(smoothed, entropy_thresholded, t, entropy_padded, MAXIMUM, NBINS, previous);
				...
				find_max(stft_real, NBINS, multiplier);
				multiplier = multiplier / initial;
				multiplier = std::min(multiplier, 1.0);

				sawtooth_filter(stft_real, scratch, smoothed, NBINS);

				fast_peaks(smoothed, entropy_thresholded, t, entropy_padded, MAXIMUM, NBINS, smoothed);
				...
				istft(stft_output, buffer, output, synthesis_window);
				return  output;
			}

			if (flag == 1) {
				// Compute product using the residual buffer since on the last round it contained data
				istft(stft_zeros, buffer, output, synthesis_window);
